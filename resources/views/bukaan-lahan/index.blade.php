...
<div>
    ...
    <div class="alert-bar">
        ...
        <div class="d-flex gap-2">
            <a class="btn" href="{{ route('api.export.bukaanlahan', request()->query()) }}">
    ...
    <div class="filter-card">
        <div class="card-body">
            <form method="GET" action="{{ route('bukaan-lahan') }}" class="row g-3 align-items-end">
                ...
                <div class="col-md-1 col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('bukaan-lahan') }}" class="btn btn-outline-secondary w-100" title="Reset">
    ...
    <div class="data-card">
        <div class="card-header">
            ...
            <a href="{{ route('bukaan-lahan.create') }}" class="btn bg-gradient-primary ms-auto">
        ...
        <div class="table-container">
            <table class="table-custom">
                ...
                <tbody>
                    @forelse ($bukaanLahanData as $data)
                    <tr>
                        <td class="ps-3">
                            <span class="row-number">
                                {{ ($bukaanLahanData->currentPage() - 1) * $bukaanLahanData->perPage() + $loop->iteration }}
                            </span>
                        </td>
                        <td>
                            <div class="location-info">
                                <h6>{{ $data->lokasi_bukaan ?? '—' }}</h6>
                            </div>
                        </td>
                        <td>
                            <div class="date-info">
                                <p>{{ $data->tanggal_bukaan ? $data->tanggal_bukaan->format('d M Y') : '—' }}</p>
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="data-value">{{ number_format($data->luas_dibuka, 2, ',', '.') }}</span>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="action-btns">
                                <a href="{{ route('bukaan-lahan.edit', $data->id) }}" class="action-btn edit" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <button type="button" class="action-btn delete delete-btn" 
                                        data-id="{{ $data->id }}" data-nama="{{ $data->lokasi_bukaan }}" title="Hapus">
        ...
        <div class="card-footer">
            <div class="d-md-flex justify-content-between align-items-center">
                <p class="text-secondary mb-3 mb-md-0">
                    Menampilkan <strong style="color: #1171ef;">{{ $bukaanLahanData->firstItem() ?? 0 }}</strong> - 
                    <strong style="color: #1171ef;">{{ $bukaanLahanData->lastItem() ?? 0 }}</strong> 
                    dari <strong style="color: #1171ef;">{{ $bukaanLahanData->total() }}</strong> data
                </p>
                <div class="pagination-container">
                    {{ $bukaanLahanData->appends(request()->query())->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus data bukaan lahan dari lokasi:</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.addEventListener('click', function (e) {
        if (e.target.closest('.delete-btn')) {
            ...
            document.getElementById('deleteForm').action = '/bukaan-lahan/' + id;
