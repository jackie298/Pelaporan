...
<style>
    ...
    .table-custom .plant-info h6 {
        color: #344767 !important; font-weight: 600; font-size: 0.9rem; margin: 0 0 4px;
    }
    .table-custom .seed-count {
        font-weight: 700; font-size: 0.95rem; color: #344767 !important;
    }
    ...
    .table-custom .status-badge.bg-warning {
    ...
    .table-custom .status-badge.bg-secondary {
        background: rgba(103, 116, 142, 0.15) !important; color: #67748e !important;
    }
    .table-custom .status-badge i { font-size: 0.7rem; }

    .table-custom .date-info p {
        margin: 0; font-weight: 600; font-size: 0.85rem; color: #344767 !important;
    }
    .table-custom .date-info .estimasi {
        color: var(--text-secondary); font-size: 0.7rem; display: flex; align-items: center; gap: 4px;
    }
...
<div>
    ...
    <div class="alert-bar">
        ...
        <div class="d-flex gap-2">
            <a class="btn" href="{{ route('api.export.nursery', request()->query()) }}">
    ...
    <div class="filter-card">
        <div class="card-body">
            <form method="GET" action="{{ route('nursery') }}" class="row g-3 align-items-end">
                <div class="col-md-3 col-6">
                    <label class="form-label"><i class="fas fa-search"></i> Cari Data</label>
                    <input type="text" name="search" class="form-control" 
                           placeholder="Jenis tanaman / Lokasi..." value="{{ request('search') }}">
                </div>
                <div class="col-md-3 col-6">
                    ...
                    <select name="status" class="form-select">
                        ...
                        <option value="sedang" {{ request('status') === 'sedang' ? 'selected' : '' }}>⚠️ Sedang</option>
                        <option value="buruk" {{ request('status') === 'buruk' ? 'selected' : '' }}>❌ Buruk</option>
                    </select>
                </div>
                <div class="col-md-2 col-6">
                    <label class="form-label"><i class="fas fa-calendar-alt"></i> Dari</label>
                ...
                <div class="col-md-2 col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('nursery') }}" class="btn btn-outline-secondary w-100" title="Reset">
    ...
    <div class="data-card">
        <div class="card-header">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <h5>
                    ...
                    Daftar Data Pembibitan
                    @if(request()->anyFilled(['search', 'status', 'tanggal_dari', 'tanggal_sampai']))
                        <span class="filter-badge"><i class="fas fa-filter me-1"></i>Filtered</span>
                    @endif
                </h5>
            </div>
            <a href="{{ route('nursery.create') }}" class="btn bg-gradient-primary ms-auto">
            ...
            <table class="table-custom">
                <thead>
                    <tr>
                        ...
                        <th class="text-center d-none d-md-table-cell">Penyemaian</th>
                    ...
                    <tr>
                        <td class="ps-3">
                            <span class="row-number">
                                {{ ($nurseryData->currentPage() - 1) * $nurseryData->perPage() + $loop->iteration }}
                            </span>
                        </td>
                        <td>
                            <div class="plant-info">
                                <h6>{{ $data->jenis_tanaman ?? '—' }}</h6>
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="seed-count">{{ number_format($data->jumlah_bibit, 0, ',', '.') }}</span>
                        </td>
                        <td>
                            <p class="text-sm mb-0">{{ $data->lokasi_pembibitan ?? '—' }}</p>
                        </td>
                        <td class="text-center">
                            @if($data->status_pertumbuhan === 'bagus')
                                <span class="status-badge bg-success">
                                    <i class="fas fa-check"></i> Bagus
                                </span>
                            @elseif($data->status_pertumbuhan === 'sedang')
                                <span class="status-badge bg-warning">
                                    <i class="fas fa-minus"></i> Sedang
                                </span>
                            @elseif($data->status_pertumbuhan === 'buruk')
                                <span class="status-badge bg-danger">
                                    <i class="fas fa-times"></i> Buruk
                                </span>
                            @else
                        ...
                        <td class="text-center d-none d-md-table-cell">
                            <div class="date-info">
                                <p>{{ $data->tanggal_penyemaian ? $data->tanggal_penyemaian->format('d M Y') : '—' }}</p>
                                @if($data->estimasi_siap_tanam)
                                    <span class="estimasi">
                                        <i class="fas fa-clock"></i> Est: {{ $data->estimasi_siap_tanam->format('d M Y') }}
                                    </span>
                                @endif
                            </div>
                        </td>
                        <td class="text-center">
                            <div class="action-btns">
                                <a href="{{ route('nursery.edit', $data->id) }}" class="action-btn edit" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <button type="button" class="action-btn delete delete-btn" 
                                        data-id="{{ $data->id }}" data-nama="{{ $data->jenis_tanaman }}" title="Hapus">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <a href="{{ route('nursery.create') }}" class="btn bg-gradient-primary btn-sm mt-2">
        ...
        @if($nurseryData->hasPages())
        <div class="card-footer">
            <div class="d-md-flex justify-content-between align-items-center">
                <p class="text-secondary mb-3 mb-md-0">
                    Menampilkan <strong style="color: #1171ef;">{{ $nurseryData->firstItem() ?? 0 }}</strong> - 
                    <strong style="color: #1171ef;">{{ $nurseryData->lastItem() ?? 0 }}</strong> 
                    dari <strong style="color: #1171ef;">{{ $nurseryData->total() }}</strong> data
                </p>
                <div class="pagination-container">
                    {{ $nurseryData->appends(request()->query())->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus data pembibitan:</p>
