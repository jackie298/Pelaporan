...
<style>
    ...
    .table-custom .triwulan-value {
        font-weight: 600; font-size: 0.85rem; color: #344767 !important;
    }
    .table-custom .triwulan-value.empty {
...
<div>
    ...
    <div class="alert-bar">
        ...
        <div class="d-flex gap-2">
            <a class="btn" href="{{ route('api.export.monitoring-vegetasi', request()->query()) }}">
    ...
    <div class="filter-card">
        <div class="card-body">
            <form method="GET" action="{{ route('monitoring-vegetasi') }}" class="row g-3 align-items-end">
                <div class="col-md-3 col-6">
                    <label class="form-label"><i class="fas fa-search"></i> Cari Data</label>
                    <input type="text" name="search" class="form-control" 
                           placeholder="Lokasi / Titik / Tanaman..." value="{{ request('search') }}">
                </div>
                <div class="col-md-2 col-6">
                    <label class="form-label"><i class="fas fa-calendar-alt"></i> Tahun</label>
                    <input type="number" name="tahun" class="form-control" 
                           placeholder="Contoh: 2024" value="{{ request('tahun') }}" min="2020" max="2099">
                </div>
                <div class="col-md-3 col-12 d-flex gap-2">
                    <button type="submit" class="btn bg-gradient-info w-100" title="Cari">
                        <i class="fas fa-search"></i>
                    </button>
                    <a href="{{ route('monitoring-vegetasi') }}" class="btn btn-outline-secondary w-100" title="Reset">
    ...
    <div class="data-card">
        <div class="card-header">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <h5>
                    ...
                    Daftar Data Monitoring Vegetasi
                    @if(request()->anyFilled(['search', 'tahun']))
                        <span class="filter-badge"><i class="fas fa-filter me-1"></i>Filtered</span>
                    @endif
                </h5>
            </div>
            <a href="{{ route('monitoring-vegetasi.create') }}" class="btn bg-gradient-primary ms-auto">
            ...
            <table class="table-custom">
                <thead>
                    <tr>
                        ...
                        <th class="text-center d-none d-md-table-cell">Triwulan III</th>
                        <th class="text-center d-none d-md-table-cell">Triwulan IV</th>
                    ...
                    <tr>
                        <td class="ps-3">
                            <span class="row-number">
                                {{ ($monitoringData->currentPage() - 1) * $monitoringData->perPage() + $loop->iteration }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td>
                            <p class="text-sm mb-0">{{ $data->titik_pantau ?? '—' }}</p>
                        </td>
                        <td>
                            <p class="text-sm mb-0">{{ $data->jenis_tanaman ?? '—' }}</p>
                        </td>
                        <td class="text-center">
                            <span class="triwulan-value {{ empty($data->tinggi_triwulan1) ? 'empty' : '' }}">
                                {{ $data->tinggi_triwulan1 ?? '—' }}
                            </span>
                        </td>
                        <td class="text-center">
                            <span class="triwulan-value {{ empty($data->tinggi_triwulan2) ? 'empty' : '' }}">
                                {{ $data->tinggi_triwulan2 ?? '—' }}
                            </span>
                        </td>
                        <td class="text-center d-none d-md-table-cell">
                            <span class="triwulan-value {{ empty($data->tinggi_triwulan3) ? 'empty' : '' }}">
                                {{ $data->tinggi_triwulan3 ?? '—' }}
                            </span>
                        </td>
                        <td class="text-center d-none d-md-table-cell">
                            <span class="triwulan-value {{ empty($data->tinggi_triwulan4) ? 'empty' : '' }}">
                        ...
                        <td class="text-center">
                            <div class="action-btns">
                                <a href="{{ route('monitoring-vegetasi.edit', $data->id) }}" class="action-btn edit" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <button type="button" class="action-btn delete delete-btn" 
                                        data-id="{{ $data->id }}" data-nama="{{ $data->lokasi }}" title="Hapus">
                    ...
                    <tr>
                        <td colspan="10">
                            <div class="empty-state">
                                ...
                                <a href="{{ route('monitoring-vegetasi.create') }}" class="btn bg-gradient-primary btn-sm mt-2">
        ...
        @if($monitoringData->hasPages())
        <div class="card-footer">
            <div class="d-md-flex justify-content-between align-items-center">
                <p class="text-secondary mb-3 mb-md-0">
                    Menampilkan <strong style="color: #1171ef;">{{ $monitoringData->firstItem() ?? 0 }}</strong> - 
                    <strong style="color: #1171ef;">{{ $monitoringData->lastItem() ?? 0 }}</strong> 
                    dari <strong style="color: #1171ef;">{{ $monitoringData->total() }}</strong> data
                </p>
                <div class="pagination-container">
                    {{ $monitoringData->appends(request()->query())->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
        @endif
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus data monitoring vegetasi dari lokasi:</p>
