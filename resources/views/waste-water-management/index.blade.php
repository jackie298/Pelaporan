...
<style>
    ...
    .table-custom .sampler-badge.inlet {
        background: rgba(17, 113, 239, 0.15);
        color: #1171ef !important;
    }

    .table-custom .sampler-badge.outlet {
        background: rgba(45, 206, 137, 0.15);
        color: #2dce89 !important;
    }

    .table-custom .sampler-badge i {
        width: 6px;
        height: 6px;
        border-radius: 50%;
    ...
    .table-custom .sampler-badge.inlet i { background: #1171ef; }
    .table-custom .sampler-badge.outlet i { background: #2dce89; }
    ...
    .table-custom .date-info .weather {
        color: #67748e !important;
        font-size: 0.75rem;
        display: flex;
        align-items: center;
    ...
    .table-custom .status-badge.bg-success {
        background: rgba(45, 206, 137, 0.15) !important;
        color: #2dce89 !important;
    }

    .table-custom .status-badge.bg-danger {
        background: rgba(245, 54, 92, 0.15) !important;
        color: #f5365c !important;
    }

    .table-custom .status-badge i {
        font-size: 0.7rem;
    }
...
<div>
    ...
    <div class="alert-bar">
        <span class="text-white">
            ...
            <strong>Waste Water Management</strong>
        </span>
        <div class="d-flex gap-2">
            <a class="btn" href="{{ route('api.export.wastewater', request()->query()) }}">
    ...
    <div class="filter-card">
        <div class="card-body">
            <form method="GET" action="{{ route('waste-water-management') }}" class="row g-3 align-items-end">
                <div class="col-md-3 col-6">
                    <label class="form-label">
                        <i class="fas fa-map-marker-alt"></i> Lokasi Sampling
                    </label>
                    <select name="lokasi" class="form-select">
                        <option value="">Semua Lokasi</option>
                        <option value="Settling Pond Rey Nabila" {{ request('lokasi') == 'Settling Pond Rey Nabila' ? 'selected' : '' }}>Settling Pond Rey Nabila</option>
                        <option value="Settling Pond Jetty Lama" {{ request('lokasi') == 'Settling Pond Jetty Lama' ? 'selected' : '' }}>Settling Pond Jetty Lama</option>
                    </select>
                </div>
                <div class="col-md-2 col-6">
                    <label class="form-label">
                        <i class="fas fa-filter"></i> Titik Sampler
                    </label>
                    <select name="sampler" class="form-select">
                        <option value="">Semua</option>
                        <option value="inlet" {{ request('sampler') == 'inlet' ? 'selected' : '' }}>Inlet</option>
                        <option value="outlet" {{ request('sampler') == 'outlet' ? 'selected' : '' }}>Outlet</option>
                    </select>
                </div>
                <div class="col-md-2 col-6">
                    <label class="form-label">
                ...
                <div class="col-md-3 col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('waste-water-management') }}" class="btn btn-outline-secondary w-100">
        ...
        <div class="card-header">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <h5>
                    ...
                    Daftar Data Pengelolaan Air Limbah
                    @if(request()->anyFilled(['lokasi', 'sampler', 'tanggal_dari', 'tanggal_sampai']))
            ...
            <a href="{{ route('waste-water-management.create') }}" class="btn bg-gradient-primary ms-auto">
            ...
            <table class="table-custom">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Debit (m³/s)</th>
                        <th class="text-center d-none d-md-table-cell">Status</th>
                        <th class="text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($wasteWaterData as $data)
                    <tr>
                        <td class="ps-3">
                            <span class="row-number">{{ ($wasteWaterData->currentPage() - 1) * $wasteWaterData->perPage() + $loop->iteration }}</span>
                        </td>
                        <td>
                            <div class="location-info">
                                <h6>{{ $data->lokasi_sampling ?? '—' }}</h6>
                                <span class="sampler-badge {{ $data->sampler }}">
                                    <i></i> {{ ucfirst($data->sampler) }}
                                </span>
                            </div>
                        </td>
                        <td>
                            <div class="date-info">
                                <p>{{ $data->tanggal_sampling ? $data->tanggal_sampling->format('d M Y') : '—' }}</p>
                                <span class="weather">
                                    <i class="fas fa-sun"></i> {{ $data->cuaca ?? '—' }}
                                </span>
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="data-value">{{ $data->ph !== null ? number_format($data->ph, 1) : '—' }}</span>
                        </td>
                        <td class="text-center">
                            <span class="data-value">{{ $data->tss !== null ? number_format($data->tss, 2) : '—' }}</span>
                        </td>
                        <td class="text-center">
                            <span class="data-value debit">{{ $data->debit_air !== null ? number_format($data->debit_air, 2) : '—' }}</span>
                        </td>
                        <td class="text-center d-none d-md-table-cell">
                            <span class="status-badge {{ $data->status_kesesuaian === 'memenuhi' ? 'bg-success' : 'bg-danger' }}">
                                <i class="fas fa-{{ $data->status_kesesuaian === 'memenuhi' ? 'check' : 'times' }}"></i>
                                {{ $data->status_kesesuaian === 'memenuhi' ? 'Memenuhi' : 'Tidak' }}
                            </span>
                        </td>
                        <td class="text-center">
                            <div class="action-btns">
                                <a href="{{ route('waste-water-management.edit', $data->id) }}" class="action-btn edit" title="Edit Data">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <button type="button" class="action-btn delete delete-btn" 
                                        data-id="{{ $data->id }}" 
                                        data-nama="{{ $data->lokasi_sampling }}" 
                                        title="Hapus Data">
        ...
        <div class="card-footer">
            <div class="d-md-flex justify-content-between align-items-center">
                <p class="text-secondary mb-3 mb-md-0">
                    Menampilkan <strong style="color: #1171ef;">{{ $wasteWaterData->firstItem() ?? 0 }}</strong> - 
                    <strong style="color: #1171ef;">{{ $wasteWaterData->lastItem() ?? 0 }}</strong> 
                    dari <strong style="color: #1171ef;">{{ $wasteWaterData->total() }}</strong> data
                </p>
                <div class="pagination-container">
                    {{ $wasteWaterData->appends(request()->query())->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus data air limbah dari lokasi:</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.addEventListener('click', function (e) {
        if (e.target.closest('.delete-btn')) {
            ...
            document.getElementById('deleteForm').action = '/waste-water-management/' + id;
