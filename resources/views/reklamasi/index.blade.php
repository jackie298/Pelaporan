...
<div>
    ...
    <div class="alert-bar">
        <span class="text-white">
            ...
            <strong>Reklamasi</strong>
        </span>
        <div class="d-flex gap-2">
            <a class="btn" href="{{ route('api.export.reklamasi', request()->query()) }}">
    ...
    <div class="filter-card">
        <div class="card-body">
            <form method="GET" action="{{ route('reklamasi') }}" class="row g-3 align-items-end">
                ...
                <div class="col-md-3 col-6">
                    ...
                    <select name="status" class="form-select">
                        ...
                        <option value="tidak_sesuai" {{ request('status') === 'tidak_sesuai' ? 'selected' : '' }}>❌ Tidak Sesuai</option>
                    </select>
                </div>
                ...
                <div class="col-md-1 col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('reklamasi') }}" class="btn btn-outline-secondary w-100" title="Reset">
    ...
    <div class="data-card">
        <div class="card-header">
            ...
            <a href="{{ route('reklamasi.create') }}" class="btn bg-gradient-primary ms-auto">
        ...
        <div class="table-container">
            <table class="table-custom">
                ...
                <tbody>
                    @forelse ($reklamasiData as $data)
                    <tr>
                        <td class="ps-3">
                            <span class="row-number">
                                {{ ($reklamasiData->currentPage() - 1) * $reklamasiData->perPage() + $loop->iteration }}
                            </span>
                        </td>
                        <td>
                            <div class="location-info">
                                <h6>{{ $data->lokasi_reklamasi ?? '—' }}</h6>
                            </div>
                        </td>
                        <td>
                            <div class="date-info">
                                <p>{{ $data->tanggal_reklamasi ? $data->tanggal_reklamasi->format('d M Y') : '—' }}</p>
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="data-value">{{ number_format($data->luas_direklamasi, 2, ',', '.') }}</span>
                        </td>
                        <td class="text-center d-none d-md-table-cell">
                            @if($data->status_kesesuaian === 'sesuai')
                        ...
                        <td class="text-center">
                            <div class="action-btns">
                                <a href="{{ route('reklamasi.edit', $data->id) }}" class="action-btn edit" title="Edit">
                                    <i class="fas fa-pen"></i>
                                </a>
                                <button type="button" class="action-btn delete delete-btn" 
                                        data-id="{{ $data->id }}" data-nama="{{ $data->lokasi_reklamasi }}" title="Hapus">
        ...
        <div class="card-footer">
            <div class="d-md-flex justify-content-between align-items-center">
                <p class="text-secondary mb-3 mb-md-0">
                    Menampilkan <strong style="color: #1171ef;">{{ $reklamasiData->firstItem() ?? 0 }}</strong> - 
                    <strong style="color: #1171ef;">{{ $reklamasiData->lastItem() ?? 0 }}</strong> 
                    dari <strong style="color: #1171ef;">{{ $reklamasiData->total() }}</strong> data
                </p>
                <div class="pagination-container">
                    {{ $reklamasiData->appends(request()->query())->links('pagination::bootstrap-4') }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus data reklamasi dari lokasi:</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.addEventListener('click', function (e) {
        if (e.target.closest('.delete-btn')) {
            ...
            document.getElementById('deleteForm').action = '/reklamasi/' + id;
